...
Usage
-----
    from leaf_contour.gap_detector import GapDetector
...
Signal weights (each 0-1, aggregated as weighted average):
  1. coarse_curvature  (σ=20) — large-amplitude concavity at macro scale
...
# Make sure petiole_utils is importable when this file is called from any CWD
...
class GapCandidate:
    ...
    start_idx: int              # index in the 512-pt resampled contour
    end_idx: int
    contour_pts: np.ndarray     # subset of the resampled contour pts (N,2)
    defect_depth_px: float = 0.0
    width_px: float = 0.0
    coarse_curvature_score: float = 0.0   # 0-1
    defect_depth_score: float = 0.0       # 0-1
    aspect_ratio_score: float = 0.0       # 0-1  (narrow bite = high)
    whiteness_score: float = 0.0          # 0-1  (background = high)
    aperiodicity_score: float = 0.0       # 0-1  (isolated = high)
...
class DetectionResult:
    ...
    damage_mask: np.ndarray             # uint8 (H, W)
    gap_candidates: List[GapCandidate] = field(default_factory=list)
    morphology: str = "unknown"         # smooth | serrated | lobed | compound
    severity: float = 0.0
    is_damaged: bool = False
    debug_overlay: Optional[np.ndarray] = None  # BGR image if requested
...
# Minimum composite score for a gap to be confirmed
_DEFAULT_THRESHOLD = 0.55

# Resampling resolution for contour analysis
_N_CONTOUR_PTS = 512
...
def _curvature_at_sigma(pts: np.ndarray, sigma: float) -> np.ndarray:
    """Signed curvature after Gaussian smoothing of the contour at the given sigma."""
    n = len(pts)
...
def _adaptive_k_threshold(k_fine: np.ndarray) -> float:
    """
    Derive a data-driven threshold from the leaf's own convex curvature.

    Only the positive (convex) curvature values represent normal leaf-outline
    variability.  We set the threshold at -(mean + 2.5 × std) of those values,
    meaning only concavities deeper than 2.5σ beyond normal variability are flagged.
    """
    positive = k_fine[k_fine > 0]
    if len(positive) < 10:
        return -0.04   # fallback to original fixed value
...
def _aperiodicity_score(damaged_flags: np.ndarray) -> float:
    """
    Compute how aperiodic the flagged concavities are.

    Returns 1.0 for an isolated gap (real herbivory) and 0.0 for many evenly-spaced
    gaps (serrations).  Based on the number of distinct flagged runs:

      1-2 runs → isolated → score 1.0
      3-7 runs → ambiguous → linear decay
      8+ runs  → periodic/serrated → score 0.0
    """
    if not np.any(damaged_flags):
        return 1.0

    # Count distinct runs of True values
    changes = np.diff(damaged_flags.astype(int))
    n_runs = int((changes == 1).sum())
    if damaged_flags[0]:
        n_runs += 1  # account for a run starting at index 0
...
def _whiteness_in_region(
    ...
    """
    Sample mean pixel lightness inside the gap polygon.

    Returns value in [0,1] where 1 = pure white background, 0 = dark tissue.
    Returns 0.5 (neutral) if no rgb image is available.
    """
...
def classify_morphology(
    ...
    Uses the perimeter-to-hull-perimeter ratio as the primary serration indicator.
    This is robust to herbivory damage: bites reduce perimeter slightly while
    serrations systematically increase it relative to the convex hull.

    Returns one of: "smooth", "serrated", "lobed", "compound"
    """
    mask_u8 = (leaf_mask > 0).astype(np.uint8) * 255

    # Check for compound leaf: multiple disconnected components
    n_labels, _, stats, _ = cv2.connectedComponentsWithStats(mask_u8, connectivity=8)
    if n_labels > 2:   # label 0 = background
    ...
    # Perimeter complexity ratio: serrations add extra edge relative to the hull.
    # A smooth leaf has ratio ≈ 1.0–1.15 (even with damage).
    # A serrated leaf has ratio > 1.25–1.40.
    # A deeply lobed leaf has ratio > 1.40.
...
class GapDetector:
    ...
    Parameters
    ----------
    leaf_mask  : uint8 or bool (H, W) — binary leaf segmentation
    rgb_image  : uint8 (H, W, 3) RGB — original white-background image (optional).
                 Required for the whiteness signal; without it the signal is neutral (0.5).
    morphology : pre-computed morphology string, or None to auto-classify.
    """
    ...
    def __init__(
        ...
        # Convexity defects from the smoothed (de-serrated) contour
        # _defect_cnt is the contour the defect indices refer to (NOT self._contour)
    ...
    def detect(
        ...
        """
        Run full gap detection pipeline.

        Parameters
        ----------
        threshold        : composite score cutoff to confirm a gap (default 0.55)
        min_depth_px     : minimum convexity defect depth to consider (pixels)
        min_segment_pts  : minimum contour segment length (in resampled points)
        merge_gap_pts    : merge adjacent flagged runs separated by ≤ this many points
                           (handles bites whose rectangular corners split the run)
        draw_debug       : if True and rgb_image was provided, attach a BGR overlay
    ...
    # ------------------------------------------------------------------
    # Step 1 — candidate extraction (coarse curvature segments)
    # ------------------------------------------------------------------
    ...
    def _find_coarse_curvature_candidates(
        ...
        # --- Pass 1: collect raw runs ----------------------------------------
        raw_runs: List[tuple] = []   # (start, end_inclusive)
        in_gap = False
        seg_start = 0
        for i in range(n + 1):
            idx = i % n
        ...
        # --- Pass 2: merge adjacent runs separated by ≤ merge_gap_pts --------
        merged: List[tuple] = []
        for run in raw_runs:
            if not merged:
                merged.append(run)
        ...
        # --- Pass 3: convert to GapCandidate objects -------------------------
    ...
    # ------------------------------------------------------------------
    # Step 2 — per-signal scoring
    # ------------------------------------------------------------------

    def _score_coarse_curvature(self, cand: GapCandidate) -> float:
        """Score based on coarse-scale curvature magnitude (more negative = better)."""
        ...
        # Correct formula: deeper concavity (more negative k) → higher score.
        # Range: 0 at threshold, 1 at 3× threshold magnitude.
        # (k_thresh - k_min) is positive when k_min < k_thresh (gap condition).
        score = float(np.clip(
    ...
    def _score_defect_depth(self, cand: GapCandidate, min_depth_px: float) -> float:
        """Cross-check with convexity defects — find the deepest defect in this gap region."""
        best_depth = self._best_defect_for_candidate(cand)
        if best_depth < min_depth_px:
            return 0.0
        # Normalize: min_depth_px → 0.0, 3×min_depth_px → 1.0
    ...
    def _score_aspect_ratio(self, cand: GapCandidate) -> float:
        """
        Gap width-to-depth ratio.  Narrow bites (ratio < 3) score high.
        Wide shallow lobes (ratio > 6) score low.
        """
    ...
    def _score_whiteness(self, cand: GapCandidate) -> float:
        """Verify that the gap region contains white background pixels.

        Returns 0.5 (neutral, neither confirms nor rejects) when no RGB image
        is available, so the whiteness signal does not penalize mask-only inputs.
        """
        if self._rgb is None:
            return 0.5   # neutral — no image to check against

        start_pt = self._pts[cand.start_idx]
        end_pt   = self._pts[cand.end_idx]
        # Polygon: gap contour pts + chord back to start
        poly = np.vstack([cand.contour_pts, end_pt, start_pt])
        raw = _whiteness_in_region(self._rgb, poly, self._h, self._w)
        # Scale: mean gray 0.80 → score 0.0, mean gray 1.0 → score 1.0
    ...
    def _build_damage_mask(self, candidates: List[GapCandidate]) -> np.ndarray:
        ...
        for cand in candidates:
            ...
            # Strategy 1: find the deepest matching convexity defect and draw
            # its triangle (start → far → end). This correctly covers the full gap.
            ...
            # Strategy 2 (fallback): draw the contour arc + chord and fill.
            # Used when no matching defect was found (rare).
            if best_tri is None:
        ...
        if damage.any():
            # Restrict to genuinely missing tissue
            damage = cv2.bitwise_and(damage, missing)
            # Dilate to cover the gap lip for inpainting
            k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11))
            damage = cv2.dilate(damage, k, iterations=1)
            # Re-restrict after dilation to avoid bleeding into intact tissue
            damage = cv2.bitwise_and(damage, missing)
            # Exclude petiole/apex zones
            damage[self._excl > 0] = 0

        return damage

    def _best_defect_triangle(self, cand: GapCandidate) -> tuple:
        """Return (depth, [(sx,sy),(ex,ey),(fx,fy)]) for the best matching defect, or (0, None)."""
    ...
    def _compute_defects(self) -> tuple:
        """Returns (defects, smoothed_contour).  Defect indices refer to smoothed_contour."""
        if self._contour is None or len(self._contour) < 5:
            return None, None
        # Smooth only enough to remove serrations (5-15 px) without filling real bites.
        # Use PIXEL COUNT (not pixel value sum) to avoid the ×255 inflation bug.
    ...
    def _best_defect_for_candidate(self, cand: GapCandidate) -> float:
        """Return depth (px) of the deepest convexity defect overlapping this candidate's arc.

        Matches defects by checking if the defect's start, end, OR far point falls
        within MAX_DIST pixels of any point in the candidate's contour arc.
        Uses self._defect_cnt (the smoothed contour the defect indices reference),
        NOT self._contour (original mask contour).
        """
        if self._defects is None or self._defect_cnt is None:
            return 0.0
        pts = cand.contour_pts
        ...
        MAX_DIST = 60.0   # generous tolerance

        best = 0.0
        for defect in self._defects:
            s, e, f, d = defect[0]
            depth = float(d) / 256.0
            # Coordinates in the SMOOTHED contour space (correct index target)
            key_pts = np.array([
    ...
    def _in_exclusion_zone(self, cand: GapCandidate) -> bool:
        """True if the candidate's centroid falls in a petiole or apex exclusion zone."""
    ...
    def _draw_debug(self, candidates: List[GapCandidate]) -> np.ndarray:
        ...
        for cand in candidates:
            color = (0, 200, 0) if cand.confirmed else (0, 165, 255)  # green / orange
        ...
        excl_vis[self._excl > 0] = [255, 0, 0]   # blue = excluded
...
# ---------------------------------------------------------------------------
# Convenience wrapper (backwards-compatible with preprocessing.py usage)
# ---------------------------------------------------------------------------
...
def detect_gaps(
    ...
    """
    Drop-in replacement for the old detect_damage() API in preprocessing.py.

    Returns a dict with the same keys as the original function so callers
    do not need changes beyond the import.
    """
    ...
    # Rebuild contour_gap_indices in the original format (list of index arrays)
    gap_indices = [
