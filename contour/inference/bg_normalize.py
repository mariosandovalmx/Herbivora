"""
Normalizacion de fondo para imagenes de herbario (papel crema/gris -> blanco uniforme).

Usado por whitebg_masks.py y predict_leaf_roi.py para fondo blanco uniforme.
"""
...
def estimate_paper_color_bgr(bgr: np.ndarray, border_frac: float = 0.08) -> np.ndarray:
    """Color medio del papel muestreando un borde de la imagen (BGR float)."""
    h, w = bgr.shape[:2]
